...
fn if_for_addr(remote_addr: net::Ipv4Addr) -> io::Result<get_if_addrs::Ifv4Addr> {
    ...
    for addr in get_if_addrs::get_if_addrs()? {
        match addr.addr {
...
fn main() -> io::Result<()> {
    ...
    let data = String::from_utf8(output.stdout).map_err(|e| io::Error::new(io::ErrorKind::InvalidData, e))?;
    ...
    let (port, secret) = data.ok_or_else(|| io::Error::new(io::ErrorKind::InvalidData, "malformated connection data"))?;
    ...
    let external_port = gateway.add_any_port(igd::PortMappingProtocol::UDP, address, 0, "mosh-server")
        .map_err(|e| io::Error::new(io::ErrorKind::Other, e))?;
